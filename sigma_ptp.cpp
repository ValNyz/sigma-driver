...
the USB connection is shut down or the CloseApplication instruction is received.
...
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetCamCaptStatus), {}, nullptr, true);
    CamCaptStatus s;
    s.decode(r.data);
    return s;
}
...
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetCamCaptStatus), {image_id}, nullptr, true);
    CamCaptStatus s;
    s.decode(r.data);
    return s;
}

CamCaptStatus SigmaCamera::wait_completion(std::uint8_t image_id,
                                           int polls, int sleep_ms)
{
    ...
    for (int i = 0; i < polls; ++i)
    {
        ...
        LOG_DEBUG("CaptStatus img=%u head=%u tail=%u code=0x%04X",
                  st.ImageId, st.ImageDBHead, st.ImageDBTail, code);
    ...
    data (SnapCommand): a snap command object.*/
{
    ...
    (void)transact(static_cast<std::uint16_t>(SigmaOp::SnapCommand), {}, &payload, false);
}
...
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetPictFileInfo2), {}, nullptr, true);
    PictFileInfo2 info;
    info.decode(r.data);
    return info;
}
...
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetPictFileInfo2), {object_handle}, nullptr, true);
    PictFileInfo2 info;
    info.decode(r.data);
    return info;
}
...
// --- BigPartialPictFile ---
BigPartialPictFile SigmaCamera::get_big_partial_pict_file(std::uint32_t address, std::uint32_t max_bytes)
...
Args:
    ...
    max_bytes (int): Image file transfer size (units: bytes / Maximum value: 0x8000000)

Returns:
    BigPartialPictFile: BigPartialPictFile object.*/
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetBigPartialPictFile), {address, 0, max_bytes}, nullptr, true);
...
{
    auto r = transact(static_cast<std::uint16_t>(SigmaOp::GetViewFrame), {}, nullptr, true);
    ViewFrame f;
    f.decode(r.data);
    return f;
}
...
std::vector<std::uint8_t> SigmaCamera::get_object_vendor(std::uint32_t object_handle, std::uint32_t chunk)
{
...
//TODO remove ?
std::vector<uint8_t> SigmaCamera::get_latest_image(DestToSave mode, int timeout)
{
