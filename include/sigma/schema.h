...
class CamDataGroupFocus
{
public:
    // Fields (same as python class)
    std::optional<FocusMode> focusMode;
    std::optional<AFLock> afLock;
    std::optional<FaceEyeAF> faceEyeAF;
    std::optional<FaceEyeAFStatus> faceEyeAFStatus; // read-only
    std::optional<FocusArea> focusArea;
    std::optional<OnePointSelection> onePointSelection;
    std::optional<uint8_t> dmfSize;
    std::optional<std::vector<uint8_t>> dmfPos;       // array of uint8
    std::optional<std::vector<uint8_t>> dmfDetection; // array of uint8, read-only
    std::optional<PreConstAF> preConstAF;
    std::optional<FocusLimit> focusLimit;

    // Build a directory blob
    std::vector<uint8_t> encode() const;

    // Parse a directory blob
    void decode(const std::vector<uint8_t> &raw);
...
private:
    ...
    void reset_()
    {
        ...
        afLock.reset();
        faceEyeAF.reset();
        faceEyeAFStatus.reset();
        focusArea.reset();
        onePointSelection.reset();
        dmfSize.reset();
        dmfPos.reset();
        dmfDetection.reset();
        preConstAF.reset();
