...
CameraPTP::Response CameraPTP::transact(std::uint16_t opcode,
                                        const std::vector<std::uint32_t> &params,
                                        const std::vector<std::uint8_t> *data_out,
                                        bool expect_data_in)
{
    std::vector<std::uint8_t> cmd(sizeof(PtpContainerHeader));
    auto *ch = reinterpret_cast<PtpContainerHeader *>(cmd.data());
    ch->total_length_bytes = std::uint32_t(sizeof(PtpContainerHeader) + params.size() * 4);
    ...
    if (data_out)
    {
        ...
        dh->total_length_bytes = std::uint32_t(sizeof(PtpContainerHeader) + data_out->size());
    ...
    if (h->container_type == PTP_CONTAINER_DATA)
    {
        ...
        {
            std::uint32_t v = std::uint32_t(p[0]) | (std::uint32_t(p[1]) << 8) | (std::uint32_t(p[2]) << 16) | (std::uint32_t(p[3]) << 24);
    ...
    // If RESPONSE directly, accept it (some bodies skip DATA even when you “expect” it)
...
std::optional<std::uint32_t> CameraPTP::wait_object_added(int timeout_ms, int poll_ms)
{
    ...
    for (int i = 0; i < tries; ++i)
    {
        ...
        if (ev.size() >= 16)
        {
            ...
            {
                std::uint32_t handle = ev[12] | (ev[13] << 8) | (ev[14] << 16) | (ev[15] << 24);
                return handle;
            }
        }
    }
...
std::vector<std::uint32_t> CameraPTP::get_storage_ids()
{
    ...
    {
        std::uint32_t v = std::uint32_t(d[i]) | (std::uint32_t(d[i + 1]) << 8) | (std::uint32_t(d[i + 2]) << 16) | (std::uint32_t(d[i + 3]) << 24);
        ids.push_back(v);
    }
    return ids;
}

std::vector<std::uint8_t> CameraPTP::get_storage_info(std::uint32_t storage_id)
{
    return transact(PTP_OP_GetStorageInfo, {storage_id}, nullptr, true).data;
}

std::uint32_t CameraPTP::get_num_objects(std::uint32_t storage, std::uint32_t format, std::uint32_t assoc)
{
    auto r = transact(PTP_OP_GetNumObjects, {storage, format, assoc}, nullptr, false);
...
std::vector<std::uint32_t> CameraPTP::get_object_handles(std::uint32_t storage, std::uint32_t format, std::uint32_t assoc)
{
    // Ask for all formats and all associations
    auto d = transact(PTP_OP_GetObjectHandles, {storage, format, assoc}, nullptr, true).data;
    ...
    std::uint32_t n = std::uint32_t(d[0]) | (std::uint32_t(d[1]) << 8) | (std::uint32_t(d[2]) << 16) | (std::uint32_t(d[3]) << 24);
    ...
    if (4 + n * 4 <= d.size())
    {
        ...
        {
            out.push_back(std::uint32_t(p[0]) | (std::uint32_t(p[1]) << 8) | (std::uint32_t(p[2]) << 16) | (std::uint32_t(p[3]) << 24));
        }
        return out;
    }
    ...
    for (size_t i = 0; i + 3 < d.size(); i += 4)
    {
        out.push_back(std::uint32_t(d[i]) | (std::uint32_t(d[i + 1]) << 8) | (std::uint32_t(d[i + 2]) << 16) | (std::uint32_t(d[i + 3]) << 24));
    }
    return out;
}
...
std::vector<std::uint8_t> CameraPTP::get_partial_object(std::uint32_t handle, std::uint32_t offset, std::uint32_t max_bytes)
{
    return transact(PTP_OP_GetPartialObject, {handle, offset, max_bytes}, nullptr, true).data;
}
...
void CameraPTP::send_object_info(const std::vector<std::uint8_t> &info_dataset)
{
...
void CameraPTP::move_object(std::uint32_t handle, std::uint32_t storage, std::uint32_t parent)
{
    (void)transact(PTP_OP_MoveObject, {handle, storage, parent}, nullptr, false);
}

void CameraPTP::copy_object(std::uint32_t handle, std::uint32_t storage, std::uint32_t parent)
{
...
std::vector<std::uint8_t> CameraPTP::get_device_prop_desc(std::uint16_t prop_code)
{
    return transact(PTP_OP_GetDevicePropDesc, {prop_code}, nullptr, true).data;
}

std::vector<std::uint8_t> CameraPTP::get_device_prop_value(std::uint16_t prop_code)
{
    return transact(PTP_OP_GetDevicePropValue, {prop_code}, nullptr, true).data;
}

void CameraPTP::set_device_prop_value(std::uint16_t prop_code, const std::vector<std::uint8_t> &raw)
{
...
// PTPy-like helpers
std::vector<std::uint8_t> CameraPTP::mesg(std::uint16_t opcode, const std::vector<std::uint32_t> &params)
{
