...
inline std::vector<uint8_t> build_resp(uint16_t rc, uint32_t txn,
                                       const std::vector<uint32_t>& params = {}) {
  std::vector<uint8_t> b; b.reserve(12 + params.size()*4);
  put_32le(b, uint32_t(12 + params.size()*4));
  put_16le(b, uint16_t(3));      // RESPONSE
  put_16le(b, rc);
  put_32le(b, txn);
  for (uint32_t p : params) put_32le(b, p);
  return b;
}
...
public:
  ...
  void queue_read(const std::vector<uint8_t>& v){ rx.insert(rx.end(), v.begin(), v.end()); }

  // Transport API
  void open_first() override { open_ = true; }
  void open_vid_pid(uint16_t, uint16_t) override { open_ = true; }
  bool is_open() const override { return open_; }
  void close() override { open_ = false; writes.clear(); rx.clear(); }
...
private:
  void ensure_auto_ok(){
    if (!auto_ok || !rx.empty() || last_txn==0) return;
