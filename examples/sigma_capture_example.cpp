...
int main() {
  try {
    ...
    // CamCaptStatus st = cam.wait_completion(0, /*polls*/ 50, /*sleep_ms*/
    // 200); LOG_INFO("CamCaptStatus: id=%u head=%u tail=%u code=0x%04X
    // dest=0x%02X", 		  st.ImageId, st.ImageDBHead, st.ImageDBTail,
    // st.Status, st.Dest); Prefer: find the new handle (event or diff)
    uint32_t handle = 0;
    ...
    // BigPartialPictFile img = cam.get_big_partial_pict_file(info.FileAddress,
    // info.FileSize); cam.get_object_vendor(256 * 1024); std::vector<uint8_t>
    // bytes = img.PartialData;
